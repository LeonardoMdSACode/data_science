...
def explanatory_analysis(charges_data_path, personal_data_path, plan_data_path):
    ...
    mc_trimmed = charges_data['monthlyCharges'].dropna().quantile([0.1, 0.9])
    charges_data['monthlyCharges'] = charges_data['monthlyCharges'].fillna(round(charges_data['monthlyCharges'].dropna().clip(mc_trimmed.iloc[0], mc_trimmed.iloc[1]).mean()))
